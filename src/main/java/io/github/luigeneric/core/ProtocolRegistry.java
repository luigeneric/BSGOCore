...
public class ProtocolRegistry implements IProtocolRegistry
{
    ...
    public ProtocolRegistry(final GameServerParamsConfig gameServerParams,
                            final Galaxy galaxy,
    ...
    )
    {
        ...
        //this.scheduledExecutorService.execute(() -> Thread.currentThread().setName("ProtRegistryScheduler " + user.getPlayer().getUserID()));
        registerProtocol(
                new LoginProtocol(sessionRegistry, this.dbProviderProvider, connection,
                        this.usersContainer, this, connectionClosedSubscriber, gameServerParams, missionUpdater));
    }
    ...
    public void loginFinished(final User user)
    {
        ...
        {
            this.user = user;
            registerProtocol(new PlayerProtocol(gameServerParams, dbProviderProvider,
                    experienceToLevelAlgo, usersContainer, guildRegistry, bgoRandom, characterServices));
            ...
            registerProtocol(new ShopProtocol(gameServerParams));
            registerProtocol(new CommunityProtocol(usersContainer, partyRegistry, guildRegistry, chatAccessBlocker));
            registerProtocol(new FeedbackProtocol());
            registerProtocol(new GameProtocol(sectorRegistry, galaxy, usersContainer));
            registerProtocol(new RoomProtocol(gameServerParams));
            registerProtocol(new DialogProtocol(galaxy, bgoRandom));
            registerProtocol(new WofProtocol(bgoRandom, micrometerRegistry));
            registerProtocol(new UniverseProtocol(galaxy));
            registerProtocol(new DebugProtocol(sectorRegistry, usersContainer, bgoRandom, chatAccessBlocker, refundProcessor));
            registerProtocol(new SubscribeProtocol(usersContainer));
            registerProtocol(new RankingProtocol());
            registerProtocol(new ArenaProtocol());
            registerProtocol(new NotificationProtocol());
            registerProtocol(new StoryProtocol());
            registerProtocol(new ZoneProtocol());
        }
        finally
        {
    ...
    private void registerProtocol(final BgoProtocol bgoProtocol)
    {
        if (!this.protocols.containsKey(bgoProtocol.getProtocolID()))
        {
            ...
            bgoProtocol.injectScheduler(scheduledService);
        }
